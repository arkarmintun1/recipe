...
it("has a route handler for fetching specific recipe", async () => {
  const recipeId = new mongoose.Types.ObjectId().toHexString();
  const response = await request(app).get(`/api/recipes/${recipeId}`).send({});
...
it("returns 200 after fetching successfully", async () => {
  ...
  await request(app).get(`/api/recipes/${recipeId}`).send({}).expect(200);
});
